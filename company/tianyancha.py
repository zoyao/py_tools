...
class Tianyancha:
    ...
    def get_basic_info(self):
        ...
        try:
            ...
            self.driver.get(url)
            table_body = self.driver.find_element(By.CLASS_NAME, "page-container")
            name = table_body.find_elements(By.TAG_NAME, "h1")[0].text
            list = table_body.find_elements(By.TAG_NAME, "span")
            for index in range(0, len(list)):
                key = list[index].text
                if key == '电话：':
                    return name, list[index + 1].text
            return name, '未知'
        except:
