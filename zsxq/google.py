...
def run(pw: Playwright) -> None:
    try:
        ...
        global first
        while True:
            url = 'https://www.google.com/search?q=' + keyword + '+site:' + site + '&filter=0'
            url = 'https://www.google.com/search?q=1&filter=0'
            print(url)
            page.goto(url)
            ...
            for i in range(100):
                ...
                more_list = page.locator('//*[@id="botstuff"]/div/div[4]/div[4]/a[1]/h3/div/span[2]')
                # more_list = page.get_by_text('更多结果').all()
                if more_list.count() > 0:
                    time.sleep(1)
                    more_list = page.locator('//*[@id="botstuff"]/div/div[4]/div[4]/a[1]/h3/div/span[2]')
                    if more_list.count() > 0:
                        more_list.last.click()
                    time.sleep(1)



            button_list = page.get_by_text('重新搜索以显示省略的结果').all()
